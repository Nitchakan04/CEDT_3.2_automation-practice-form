...
import { TEST_DOB, TEST_STATE_CITY, TEST_SUBJECTS } from "../data/registration.data";
...
test.describe("Student Registration Form - Widget & UI", () => {
  ...
  test("TC02: DOB can be selected via calendar widget", async ({ page }) => {
    ...
    await expect(reg.dobInput).toHaveValue(
      new RegExp(`${String(dob.day).padStart(2, "0")}\\s+(Mar|March)\\s+${dob.year}`, "i"),
    );
  });

  test("TC03: City is disabled/empty until State is selected", async ({ page }) => {
    const reg = new RegistrationPage(page);
    await reg.goto();
  ...
  test("TC04: City changes by State (filtered options)", async ({ page }) => {
    ...
    await reg.expectCityOptionsContainOnly(TEST_STATE_CITY.NCR.city, TEST_STATE_CITY.UP.city);

    await reg.selectState(TEST_STATE_CITY.UP.state);
    await reg.expectCityOptionsContainOnly(TEST_STATE_CITY.UP.city, TEST_STATE_CITY.NCR.city);
  });
